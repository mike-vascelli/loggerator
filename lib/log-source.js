...
module.exports = class LogSource {
	...
	static compare(firstLog, secondLog) {
		const firstLogDate = firstLog ? firstLog.date : null
		const secondLogDate = secondLog ? secondLog.date : null

		if (!_.isDate(firstLogDate) || !_.isDate(secondLogDate)) {
			throw new Error('Cannot compare invalid logDates!!')
		}

		return firstLogDate.getTime() - secondLogDate.getTime()
	}
}